...
// function they call and which of its returned arrays they draw, so each is
// one spec here; addWangPanelDefinitions() turns them into normal indicator
...
const WANG_PANEL_INDICATORS = {
  ...
  // The 10+2 Guppy EMAs go on the main chart (stock-app.prod.js OVERLAY_DEFS 'GuppyMA'); the Spread is this panel.
  ...
  // Bands go on the main chart (OVERLAY_DEFS 'BollingerNew'); %B and Bandwith are this panel.
...
const WANG_EXTRA_LINE_COLORS = ['#e879f9', '#4ade80', '#f87171', '#a78bfa', '#fbbf24', '#94a3b8'];
const WANG_IMPULSE_COLORS = { Green: '#22c55e', Red: '#ef4444', Blue: '#3b82f6' };

// The lines a Wang panel draws, in order, with legend title and color - shared
// by renderWangPanel() and the legend so both agree.
function wangPanelLines(spec, colors) {
  const palette = [colors.LINE1, colors.LINE2, colors.LINE3, ...WANG_EXTRA_LINE_COLORS];
  const lines = spec.lines.map((key, i) => ({
    key,
    title: (spec.titles && spec.titles[key]) || key,
    color: palette[i % palette.length],
  }));
  return spec.withClose ? [{ key: 'Close', title: 'Close', color: '#cbd5e1' }, ...lines] : lines;
}
...
class MultiIndicatorSystem {
  ...
  constructor(options = {}) {
    ...
    this.mainChart = null;
    this.timeFormatOptions = null; // see setTimeFormatOptions()
  ...
  createIndicatorDefinitions() {
    ...
    this.addWangPanelDefinitions(definitions);
    return definitions;
  }

  // One definition per WANG_PANEL_INDICATORS spec (see the top of this file).
  addWangPanelDefinitions(definitions) {
    Object.entries(WANG_PANEL_INDICATORS).forEach(([key, spec]) => {
      if (definitions[key]) {
        console.warn(`WANG_PANEL_INDICATORS.${key} has the same key as an existing indicator - skipped`);
        return;
      }
  ...
  computeWangPanel(spec, data, params) {
    const len = data.length;
    const keys = [...spec.lines, spec.histogram, spec.colorKey].filter(Boolean);
    const out = Object.fromEntries(keys.map(k => [k, new Array(len).fill(null)]));
    if (spec.withClose) out.Close = data.map(d => d.close);
    ...
    keys.forEach(k => {
      ...
      for (let i = 0; i < len; i++) {
        const v = src[i];
        out[k][i] = k === spec.colorKey ? (v || null) : (v != null && Number.isFinite(v) ? v : null);
      }
    });
    return out;
  }

  renderWangPanel(spec, chart, data, colors, seriesMap) {
    if (!data) return;
    const priceFormat = spec.volumeScale ? { type: 'volume' }
      ...
      : null;
    const withFormat = (opts) => (priceFormat ? { ...opts, priceFormat } : opts);

    if (spec.histogram && data[spec.histogram]) {
      if (!seriesMap.has(spec.histogram)) {
        seriesMap.set(spec.histogram, chart.addHistogramSeries(withFormat({ base: 0, title: '', priceLineVisible: false })));
      }
      const barColors = spec.colorKey ? data[spec.colorKey] : null;
      seriesMap.get(spec.histogram).setData(this.seriesWithLeadInPadding(data[spec.histogram], (v, i) => {
        if (v == null || isNaN(v)) return { value: null };
        const named = barColors && WANG_IMPULSE_COLORS[barColors[i]];
        return { value: v, color: named || (v >= 0 ? colors.UP : colors.DOWN) };
      }));
    }

    wangPanelLines(spec, colors).forEach(({ key, title, color }) => {
      if (!data[key]) return;
      if (!seriesMap.has(key)) {
        seriesMap.set(key, chart.addLineSeries(withFormat({
          color,
          lineWidth: key === 'Close' ? 1 : 2,
          title,
          crosshairMarkerVisible: false,
          priceLineVisible: false,
        })));
      }
      seriesMap.get(key).setData(this.seriesWithLeadInPadding(data[key], (v) => (v == null || isNaN(v)) ? null : v));
    });
  }
  ...
  initializePanel(panelId, indicatorType) {
    ...
    const chart = LightweightCharts.createChart(chartElement, {
      ...
      // NOTE: panel sizing is handled entirely by the manual
      // ResizeObserver + _scheduleQuietRender below, which call
      // chart.resize(). Don't add autoSize here: with it on,
      // lightweight-charts v5 ignores resize() calls.
    ...
    if (this.timeFormatOptions) {
      try { chart.applyOptions(this.timeFormatOptions); } catch (e) {}
    }
  ...
  updateLegendDisplay(panelId, indicatorData = null, atIndex = null) {
    ...
    switch(panel.indicatorType) {
      ...
      case 'VOLUME':
        ...
        break;
      default: {
        const spec = WANG_PANEL_INDICATORS[panel.indicatorType];
        if (spec && indicatorData) {
          // computeWangPanel() arrays are full-length, so atIndex needs no offset.
          const fmt = (arr) => {
            const v = arr ? arr[atIndex != null ? atIndex : arr.length - 1] : null;
            if (v == null || !Number.isFinite(v)) return '-';
            if (spec.volumeScale && Math.abs(v) >= 1e3) {
              const [div, unit] = Math.abs(v) >= 1e9 ? [1e9, 'B'] : Math.abs(v) >= 1e6 ? [1e6, 'M'] : [1e3, 'K'];
              return (v / div).toFixed(2) + unit;
            }
            return v.toFixed(spec.precision || 2);
          };
          const entries = wangPanelLines(spec, this.colors);
          if (spec.histogram) entries.push({ key: spec.histogram, title: spec.histogram, color: '#888' });
          legendHTML = entries
            .map(({ key, title, color }) => `<span style="color: ${color}; margin-right: 10px;">● ${title}: ${fmt(indicatorData[key])}</span>`)
            .join('');
        } else {
  ...
  // Time-label formatters (localization.timeFormatter / timeScale.tickMarkFormatter)
  // the host app uses on its main chart, so a panel's date axis reads the same.
  // Applied to open panels now and to panels created later.
  setTimeFormatOptions(options) {
    this.timeFormatOptions = options;
    this.panels.forEach(panel => {
      if (!panel.chart) return;
      try { panel.chart.applyOptions(options); } catch (e) {}
    });
  }

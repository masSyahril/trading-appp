...
function VariableIndexDynamicAvg(STK_high, STK_low, STK_close, period) {
  ...
  for(let i=period+1; i<=STK_close.length; i++) {  //i=11 to 2000
    ...
    CMO=(sum_up+sum_down===0) ? 0 : Math.abs(sum_up-sum_down)/(sum_up+sum_down)*100;  // chat gpt correction
...
// 判斷「目前價格偏離近期平均值多少個標準差」
function Z_Score_Close(STK_close, Z_num) {
  // Menu Name: Z-Score (Close)     //Z_num=10,...
  const MA=[];  //Moving Average, =10 to 2000
  //Calculate MA[]= 10 to 2000
  let sum=0;
  for(let i=1; i<=Z_num; i++) {  //i=1 to 10(=Z_num)
    sum=sum+STK_close[i];
  }
  MA[Z_num]=sum/Z_num;   //first average MA[10]
  for(let i=Z_num+1; i<=STK_close.length; i++) {  //=11 to 2000
    sum=sum-STK_close[i-Z_num]+STK_close[i]; //減舊加新
    MA[i]=sum/Z_num;     //MA[11] to 2000
  }
  //Calculate Z_Score, Standard Deviation(SD), Z_num=10
  const Z_Score=[], SD=[];  //Standard Deviation(SD), Z_num=10, =10 to 2000
  for(let i=Z_num; i<=STK_close.length; i++) {  //=10 to 2000
    sum=0;
    for(let j=i-Z_num+1; j<=i; j++){  //j=1 to 10
      sum=sum+(STK_close[j]-MA[i])**2;
    }
    SD[i]=Math.sqrt(sum/Z_num);             //SD[]=10 to 2000
    Z_Score[i]=(STK_close[i]-MA[i])/SD[i];  //Z_Score[]=10 to 2000
  }
  return { Z_Score };
  // drawing these figures in the small windows.
  // if Z_num=10, then these indicators[]=10 to 2000.
}
window.Z_Score_Close = Z_Score_Close;
//----------------------------------------------------------------------

//===designed by Prof Wang, 2026-August-12=============(完全自行創新)========
//Z-Score(此程式用Close計算標準化分數)。
//Z-Score通常不是直接對單一價格做標準化，而是利用一段期間的歷史價格，
// 判斷「目前價格偏離近期平均值多少個標準差」
//以Typical Price取代方程式中的收盤價Close
function Z_Score_Typical(STK_high, STK_low, STK_close, Z_num) {
  // Menu Name: Z-Score (Typical)     //Z_num=10,...
  const TP=[];  //TP[]=TypicalPrice=[], =1 to 2000,以TypicalPrice取代Close
  for(let i=1; i<=STK_close.length; i++) {  //i=1 to 2000
    TP[i]=(STK_high[i]+STK_low[i]+3*STK_close[i])/5;
  }
  //Calculate MA of Typical Price(TP), Z_num=10
  const MA=[];  //Moving Average, =10 to 2000
  let sum=0;
  for(let i=1; i<=Z_num; i++) {  //i=1 to 10(=Z_num)
    sum=sum+TP[i];
  ...
  for(let i=Z_num; i<=STK_close.length; i++) {  //=10 to 2000
    ...
    SD[i]=Math.sqrt(sum/Z_num);      //SD[]=10 to 2000
    Z_Score[i]=(TP[i]-MA[i])/SD[i];  //Z_Score[]=10 to 2000
  }
  return { Z_Score };
  // drawing these figures in the small windows.
  // if Z_num=10, then these indicators[]=10 to 2000.
}
window.Z_Score_Typical = Z_Score_Typical;
//----------------------------------------------------------------------

//===designed by Prof Wang, 2026-August-12=============(完全自行創新)========
//Z-Score(此程式用EMA計算標準化分數)。
//Z-Score通常不是直接對單一價格做標準化，而是利用一段期間的歷史價格，
// 判斷「目前價格偏離近期平均值多少個標準差」
//以EMA取代方程式中的收盤價Close。而EMA的計算方式是使用Typical Price替代Close。
//指數平滑移動平均的參數:exponential smoothing parameter(esp)
function Z_Score_EMA(STK_high, STK_low, STK_close, Z_num, esp) {
  // Menu Name: Z-Score (EMA)     //Z_num=10,... // esp=9,10,...
  const TP=[];  //TP[]=TypicalPrice=[], =1 to 2000,以TypicalPrice取代Close
  for(let i=1; i<=STK_close.length; i++) {  //i=1 to 2000
    TP[i]=(STK_high[i]+STK_low[i]+3*STK_close[i])/5;
  }
  //Calculate EMA, esp=9
  const EMA=[];  //Exponential Moving Average, =1 to 2000
  EMA[1]=TP[1];  //first EMA[1]
  for(let i=2; i<=STK_close.length; i++) {  //i=2 to 2000
    EMA[i]=(esp-1)/(esp+1)*EMA[i-1]+2/(esp+1)*TP[i];  //EMA[2] to 2000
  }
  //Calculate the moving average of EMA[]. 針對EMA[1:2000]計算移動平均。
  const MA=[];  //MA[]=EMA[]的移動平均, =10 to 2000
  let sum=0;
  for(let i=1; i<=Z_num; i++) {  //i=1 to 10(=Z_num)
    sum=sum+EMA[i];
  }
  MA[Z_num]=sum/Z_num;   //first average MA[10]
  for(let i=Z_num+1; i<=STK_close.length; i++) {  //=11 to 2000
    sum=sum-EMA[i-Z_num]+EMA[i]; //減舊加新
    MA[i]=sum/Z_num;     //MA[11] to 2000
  }
  //Calculate Z_Score, Standard Deviation(SD), Z_num=10
  const Z_Score=[], SD=[];  //Standard Deviation(SD), Z_num=10, =10 to 2000
  for(let i=Z_num; i<=STK_close.length; i++) {  //=10 to 2000
    sum=0;
    for(let j=i-Z_num+1; j<=i; j++){  //j=1 to 10
      sum=sum+(EMA[j]-MA[i])**2;
    }
    SD[i]=Math.sqrt(sum/Z_num);       //SD[]=10 to 2000
    Z_Score[i]=(EMA[i]-MA[i])/SD[i];  //Z_Score[]=10 to 2000
  }
  return { Z_Score };
  // drawing these figures in the small windows.
  // if Z_num=10, then these indicators[]=10 to 2000.
}
window.Z_Score_EMA = Z_Score_EMA;
//----------------------------------------------------------------------

//===designed by Prof Wang, 2026-August-14=============(完全自行創新)========
//Stock Return Z-Score by Close Price (以收盤價計算股票報酬率Z分數)
//Z-Score(此程式用Stock Return計算標準化分數)。(股票報酬率Z分數)
//Z-Score通常不是直接對單一價格做標準化，而是利用一段期間的歷史價格，
// 判斷「目前價格偏離近期平均值多少個標準差」
//以Stock Return取代方程式中的收盤價Close。
//以Close計算Stock Return，然後再以Stock Return計算Z-Score。
function Z_Score_Close_Return(STK_close, Z_num) {
  // Menu Name: Z-Score_Close_Return     //Z_num=10,...
  //Calculate Stock Return, Z_num=10
  const STK_return=[];  //=2 to 2000. =[C(t)/C(t-1)-1]*100,股票報酬率
  for(let i=2; i<=STK_close.length; i++) {  //i=2 to 2000
    STK_return[i]=(STK_close[i]/STK_close[i-1]-1)*100;
  }
  //Calculate MA of Stock Return, Z_num=10
  const MA=[];  //Moving Average, =11 to 2000
  let sum=0;
  for(let i=2; i<=Z_num+1; i++) {  //i=2 to 10+1(=Z_num)
    sum=sum+STK_return[i];
  }
  MA[Z_num+1]=sum/Z_num;   //first MA[11] of Stock Return
  for(let i=Z_num+2; i<=STK_close.length; i++) {  //=12 to 2000
    sum=sum-STK_return[i-Z_num]+STK_return[i];    //減舊加新
    MA[i]=sum/Z_num;     //second=MA[12]
  }
  //Calculate Z_Score, Standard Deviation(SD), Z_num=10
  const Z_Score=[], SD=[];  //Standard Deviation(SD), Z_num=10, =11 to 2000
  for(let i=Z_num+1; i<=STK_close.length; i++) {  //=11 to 2000
    sum=0;
    for(let j=i-Z_num+1; j<=i; j++){  //j=2 to 10+1(=Z_num+1)
      sum=sum+(STK_return[j]-MA[i])**2;
    }
    SD[i]=Math.sqrt(sum/Z_num);      //SD[]=11 to 2000
    Z_Score[i]=(STK_return[i]-MA[i])/SD[i];  //Z_Score[]=11 to 2000
  }
  return { Z_Score };
  // drawing these figures in the small windows.
  // if Z_num=10, then these indicators[]=11 to 2000.
}
window.Z_Score_Close_Return = Z_Score_Close_Return;
//----------------------------------------------------------------------

//===designed by Prof Wang, 2026-August-14=============(完全自行創新)========
//Stock Return Z-Score by Typical Price (以典型價格計算股票報酬率Z分數)
//Z-Score(此程式用Stock Return計算標準化分數)。(股票報酬率Z分數)
//Z-Score通常不是直接對單一價格做標準化，而是利用一段期間的歷史價格，
// 判斷「目前價格偏離近期平均值多少個標準差」
//以Stock Return取代方程式中的收盤價Close。
//以TP計算Stock Return，然後再以Stock Return計算Z-Score。
function Z_Score_TP_Return(STK_high, STK_low, STK_close, Z_num) {
  // Menu Name: Z-Score_TP_Return     //Z_num=10,...
  const TP=[];  //TP[]=TypicalPrice=[], =1 to 2000,以TypicalPrice取代Close
  for(let i=1; i<=STK_close.length; i++) {  //i=1 to 2000
    TP[i]=(STK_high[i]+STK_low[i]+3*STK_close[i])/5;
  }
  //Calculate Stock Return by TP, Z_num=10
  const STK_return=[];  //=2 to 2000. =[C(t)/C(t-1)-1]*100,股票報酬率
  for(let i=2; i<=STK_close.length; i++) {  //i=2 to 2000
    STK_return[i]=(TP[i]/TP[i-1]-1)*100;
  }
  //Calculate MA of Stock Return by TP, Z_num=10
  const MA=[];  //Moving Average, =11 to 2000
  let sum=0;
  for(let i=2; i<=Z_num+1; i++) {  //i=2 to 10+1(=Z_num)
    sum=sum+STK_return[i];
